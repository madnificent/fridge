...
(defgeneric load-instance-from-column-alist (class &rest column-alist)
  (:documentation "Creates a new instance, ensuring the values from the column alist are its standard values."))
...
(defmethod load-instance-from-column-alist ((class db-support-metaclass) &rest column-alist)
  (declare (special init-columns))
  (let ((init-columns column-alist))
    (declare (special init-columns))
    (make-instance class)))

(defmethod initialize-instance :after ((object db-support-class) &key &allow-other-keys)
  (declare (special init-columns))
  (when (boundp 'init-columns)
    (apply #'set-slots-from-column-alist (class-of object) object init-columns))
  object)
