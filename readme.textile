...
* save-object-ring : ( *warning* this method is currently not supported ) You can use (save-object-ring object) which stores all objects a certain object links to, if you have modified only one object.  This ensures that you will not modify objects that are related to the given object but shouldn't have been touched (can be handy if you toyed around a bit too much).

It is assumed that you will use save-quickstore most.  If the object hasn't been changed internally (but only has been fetched to inspect its values), the quickstore will not query the database to store that certain object.
